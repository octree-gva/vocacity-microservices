...
export type ContextMeta = GenericObject & { bearer?: string; permissions?: string[] } & (
    | {
        isUserSignedIn: true;
        user: string;
      }
...
export type CanAction = (organizationId: string, role: string) => Promise<void>;
...
export type ActionJob<AJob extends Action> = Action & {
  params: {
    /* Who can read this job */
    aud: string;
    /* For which envName is the job */
    envName?: string;
  } & AJob["params"];
  returns: SerializedJob;
};
export type ParkActionJob = ActionJob<{
  params: { template: string; aud: string };
  returns: SerializedJob;
}>;
